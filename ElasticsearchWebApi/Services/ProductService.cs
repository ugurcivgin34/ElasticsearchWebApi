...
using ElasticsearchWebApi.Repositories;
using Nest;
using System.Net;
...
namespace ElasticsearchWebApi.Services
{
    public class ProductService
    {
        ...
        public async Task<ResponseDto<List<ProductDto>>> GetAllAsync()
        {
            ...
            foreach (var x in products)
            {
                if (x.Feature is null) 
                {
                    ...
                    continue; //Feature null ise diğer özellikleri almadan devam et.if else yapmak yerine continue kullanmak daha mantıklı.
                }
        ...
        public async Task<ResponseDto<bool>> DeleteAsync(string id)
        {
            ...
            {
                _logger.LogError(deleteResponse.OriginalException, deleteResponse.ServerError.Error.ToString()); //OriginalException ile hata mesajını logluyoruz.
